...
public class KafkaEventPublisher implements EventPublisher {
    ...
    public void publishDeviceSessionEvent(DeviceSessionEvent event) {
        try {
            kafkaTemplate.send(deviceSessionsTopic, event.getImei().getValue(), event)
                    .addCallback(
    ...
    public void publishTelemetryEvent(TelemetryEvent event) {
        try {
            kafkaTemplate.send(telemetryInboundTopic, event.getImei().getValue(), event)
                    .addCallback(
    ...
    public void publishCommandEvent(CommandEvent event) {
        try {
            kafkaTemplate.send(commandsOutboundTopic, event.getImei().getValue(), event)
                    .addCallback(
